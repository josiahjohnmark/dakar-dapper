/* ==========================================================================
   SOCIALS.JS — social links, shared by the storefront and the admin
   --------------------------------------------------------------------------
   Brand marks are the official glyphs from Simple Icons (CC0, v13.21.0),
   drawn at 24x24 so they stay crisp at any size and cost no extra requests.
   The list of links itself lives in the database (dd_content.socials) and is
   edited from Admin -> More -> Social links.
   ========================================================================== */

const SOCIAL_PLATFORMS = {
  instagram: {
    label: "Instagram", color: "#FF0069", fg: "#FFFFFF",
  ...
  telegram: {
    label: "Telegram", color: "#26A5E4", fg: "#FFFFFF",
  ...
  x: {
    label: "X (Twitter)", color: "#000000", fg: "#FFFFFF",
    match: /(^|\.)(twitter\.com|x\.com)$/i,
    path: "M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z"
  },
  whatsapp: {
    label: "WhatsApp", color: "#25D366", fg: "#FFFFFF",
  ...
  threads: {
    label: "Threads", color: "#000000", fg: "#FFFFFF",
  ...
  pinterest: {
    label: "Pinterest", color: "#BD081C", fg: "#FFFFFF",
    match: /(^|\.)(pinterest\.[a-z.]+|pin\.it)$/i,
  ...
  linktree: {
    label: "Linktree", color: "#43E55E", fg: "#FFFFFF",
    match: /(^|\.)linktr\.ee$/i,
  ...
  email: {
    label: "Email", color: "#1A1A1A", fg: "#FFFFFF", match: null, stroke: true,
    path: "M4 4h16a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2zm18 2-10 7L2 6"
  },
  website: {
    label: "Website", color: "#1A1A1A", fg: "#FFFFFF", match: null, stroke: true,
...
/* Which platform does a pasted link belong to? */
function detectSocialPlatform(url) {
  const s = String(url || "").trim();
  if (/^mailto:/i.test(s) || /^[^@\s/]+@[^@\s/]+\.[^@\s/]+$/.test(s)) return "email";
  let host = "";
  try { host = new URL(/^https?:\/\//i.test(s) ? s : "https://" + s).hostname.replace(/^www\.|^m\./, ""); }
  catch { return "website"; }
  for (const [id, p] of Object.entries(SOCIAL_PLATFORMS)) {
    if (p.match && p.match.test(host)) return id;
  }
  return "website";
}

/* Share links arrive full of tracking tags (?si=, utm_*, _t=, mibextid...).
   Strip them so the link is clean and stable. Short/share links that need
   their query (none of the supported ones do) are left intact. */
...
/* Only ever render https/mailto links — never javascript: or data:. */
...
function socialIconSvg(platform, size) {
  ...
  const px = size || 20;
  return p.stroke
    ? `<svg viewBox="0 0 24 24" width="${px}" height="${px}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="${p.path}"/></svg>`
    : `<svg viewBox="0 0 24 24" width="${px}" height="${px}" fill="currentColor" aria-hidden="true" focusable="false"><path d="${p.path}"/></svg>`;
}

/* The fallback list, used until the database answers (and offline). */
